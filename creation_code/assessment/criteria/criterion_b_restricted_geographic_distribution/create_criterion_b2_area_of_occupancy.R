...
##  Notes: ----
##              1. Criterion B2 measures the distribution
##                   of coral reef ecosystems based on a 
##                   standard 10x10km grid.
##                 AOO is determined by counting the number of 
##                   10x10 km cells occupied by >1 km2 of 
##                   coral reef ecosystem within Ecoregions 
##                   (or other Areas of Assessment) for region.
##              2. This process basically entails creation of 
##                   10x10km grid, calculating grid uncertainty 
##                   and getting the number of grid cells that
##                   are occupied by more than 1% by coral
##                   reef ecosystems.
...
# [10] "Western and Northern Madagascar"
 ...
 ## -- set parameters for AOO calculations -- ## ----

 ## -- Evaluation of AOO uses two key functions from      ##
 ##      the `redlistr` package:                          ##
 ##        - makeAOOGrid()                                ##
 ##        - gridUncertainty()                            ##
 ##    These functions require inputs to specify:         ##
 ##      1. Grid representing the area of occupancy       ##
 ##         for ecosystem distributions.  This is         ##
 ##         defined in variable `grid_size`.              ##
 ##      2. `p_thresh` defines the minimum percent        ##
 ##         of the grid cell to be occupied before        ##
 ##         it is counted for AOO.  Default is 1 %.       ##
 ##      3. To deal with potential variations in          ##
 ##         grid size and underlying habitat layers       ##
 ##         gridUncertainty() determines the number       ##
 ##         of AOO grid cells occupied by systematically  ##
 ##         calculating values after a set number of      ##
 ##         scenarios.  This is controled through         ##
 ##         the variable `n_improvements`              -- ##
  ...
  # classify
    criterion_b2_area_of_occupancy %<>%
      mutate(Status = ifelse(AOO <= 2, "CR", NA),
             ...
             Status = ifelse(AOO %>% between(50, 55), "NT", Status), # Added 10% from VU as threshold for NT
             Status = ifelse(AOO > 55,                "LC", Status))
# # A tibble: 10 × 4
